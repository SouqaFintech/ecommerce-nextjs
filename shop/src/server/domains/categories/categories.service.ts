...
export class CategoriesService {
  ...
  getCategories({ limit: __limit, page, search, parent }: GetCategoriesDto) {
    const limit = __limit ? Number(__limit) : 10;
    if (!page) page = 1;
  ...
  getCategory(param: string, language: string): Category | null {
    return (
      this.categories.find((p) => p.id === Number(param) || p.slug === param) ??
      null
    );
  }
